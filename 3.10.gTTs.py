...
def TTS(response, apikey):
    # gTTS를 활용하여 음성 파일 생성
    filename = '../ch03-04/output.mp3'
    client = OpenAI(api_key = apikey)
    # tts = gTTS(text=response, lang="ko") -> openai api 목소리 노바
...
def main():
    ...
    if "messages" not in st.session_state:
        st.session_state["messages"] = [{"role": "system",
                                         "content": "You are a thoughtful assistant. Respond to all input in 25 words and answer in korean language"}]

    if "check_reset" not in st.session_state: #reset 오타
    ...
    with st.sidebar:
        ...
        api_value = st.text_input(label="OPENAI API 키",
        ...
        if st.button(label="초기화"):
            ...
            st.session_state["chat"] = []
            st.session_state["messages"] = [{"role": "system", "content" : "You are a thoughtful assistant. Respond to all input in 25 words and answer in korean language"}]
    ...
    with col2:
        ...
        if (audio.duration_seconds > 0) and (st.session_state["check_reset"] == False):
            # ChatGPT에게 답변 얻기
            response = ask_gpt(st.session_state["messages"], model, api_value)
